...
const Navbar = () => {
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            const response = await axios.get('http://localhost:3007/fetch-categories');
    ...
    const handleSearch = () => {
        if (categories.map(cat => cat.toLowerCase()).includes(productSearch.toLowerCase())) {
    ...
    return (
        <>
            ...
            {loading && <div className="loading-message">Loading categories...</div>} {/* Loading message */}
        </>
    );
};
